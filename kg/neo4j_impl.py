...
class Neo4JStorage(BaseGraphStorage):
    ...
    async def initialize(self):
        async with get_data_init_lock():
            neo4j_config = self.global_config.get("storage_config", {}).get("neo4j", {})

            URI = neo4j_config.get("uri")
            USERNAME = neo4j_config.get("username")
            PASSWORD = neo4j_config.get("password")

            MAX_CONNECTION_POOL_SIZE = int(neo4j_config.get("connection_pool_size", 100))
            CONNECTION_TIMEOUT = float(neo4j_config.get("connection_timeout", 30.0))
            CONNECTION_ACQUISITION_TIMEOUT = float(neo4j_config.get("connection_acquisition_timeout", 30.0))
            MAX_TRANSACTION_RETRY_TIME = float(neo4j_config.get("max_transaction_retry_time", 30.0))
            MAX_CONNECTION_LIFETIME = float(neo4j_config.get("max_connection_lifetime", 300.0))
            LIVENESS_CHECK_TIMEOUT = float(neo4j_config.get("liveness_check_timeout", 30.0))

            keep_alive_val = neo4j_config.get("keep_alive", "true")
            if isinstance(keep_alive_val, bool):
                KEEP_ALIVE = keep_alive_val
            else:
                KEEP_ALIVE = str(keep_alive_val).lower() in ("true", "1", "yes", "on")

            DATABASE = neo4j_config.get("database", re.sub(r"[^a-zA-Z0-9-]", "-", self.namespace))
