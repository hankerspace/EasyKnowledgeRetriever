...
from easy_knowledge_retriever.constants import DEFAULT_EMBEDDING_BATCH_NUM
...
class NanoVectorDBStorage(BaseVectorStorage):
    def __post_init__(self):
        ...
        nano_workspace = os.environ.get("NANO_VECTOR_DB_WORKSPACE")
        if nano_workspace and nano_workspace.strip():
            # Use environment variable value, overriding the passed workspace parameter
            effective_workspace = nano_workspace.strip()
            logger.info(
                f"Using NANO_VECTOR_DB_WORKSPACE environment variable: '{effective_workspace}' (overriding passed workspace: '{self.workspace}')"
            )
        else:
        ...
        self._max_batch_size = DEFAULT_EMBEDDING_BATCH_NUM
    ...
    async def initialize(self):
        ...
        async with get_data_init_lock():
            ...
            try:
                ...
                logger.info(
                    f"[{self.workspace}] NanoVectorDB collection '{self.namespace}' initialized successfully"
                )
            except Exception as e:
                logger.error(
                    f"[{self.workspace}] Failed to initialize NanoVectorDB collection '{self.namespace}': {e}"
                )
                raise
