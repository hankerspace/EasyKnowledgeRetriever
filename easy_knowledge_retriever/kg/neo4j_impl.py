...
class Neo4JStorage(BaseGraphStorage):
    def __init__(self, 
                 ...
                 neo4j_max_connection_lifetime=300.0,
                 neo4j_liveness_check_timeout=30.0,
        ...
        self.neo4j_connection_pool_size = int(neo4j_connection_pool_size)
        self.neo4j_connection_timeout = float(neo4j_connection_timeout)
        self.neo4j_connection_acquisition_timeout = float(neo4j_connection_acquisition_timeout)
        self.neo4j_max_transaction_retry_time = float(neo4j_max_transaction_retry_time)
        self.neo4j_max_connection_lifetime = float(neo4j_max_connection_lifetime)
        self.neo4j_liveness_check_timeout = float(neo4j_liveness_check_timeout)
    ...
    async def initialize(self):
        async with get_data_init_lock():
            ...
            LIVENESS_CHECK_TIMEOUT = self.neo4j_liveness_check_timeout

            KEEP_ALIVE = self.neo4j_keep_alive

            database = self.neo4j_database
            DATABASE = database if database else re.sub(r"[^a-zA-Z0-9-]", "-", self.namespace)
    ...
    async def get_knowledge_graph(
        ...
        """
        # Get max_nodes from self.max_graph_nodes if not provided
